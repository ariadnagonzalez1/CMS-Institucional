{{-- resources/views/admin/albumes/edit.blade.php --}}
@extends('layouts.admin')

@section('title', 'Editar Álbum')

@section('header-title', 'Editar Álbum')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm">
        <form action="{{ route('admin.albumes.update', $album) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="descripcion" 
                              ...
                              placeholder="Describa el contenido del álbum...">{{ old('descripcion', $album->descripcion) }}</textarea>
                </div>

                <div class="flex gap-6">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="visible" value="1" class="rounded border-gray-300 text-[#1a3b2e]" {{ old('visible', $album->visible) ? 'checked' : '' }}>
                        <span class="ml-2 text-sm text-gray-700">Visible en el sitio</span>
                    </label>
                    
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="estado" value="1" class="rounded border-gray-300 text-[#1a3b2e]" {{ old('estado', $album->estado) ? 'checked' : '' }}>
