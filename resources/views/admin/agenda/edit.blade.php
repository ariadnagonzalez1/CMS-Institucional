...
@section('header-title', 'Editar Evento')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm">
        <form action="{{ route('admin.agenda.update', $agendum) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="seccion_agenda_id" 
                            ...
                            @foreach($secciones as $seccion)
                                <option value="{{ $seccion->id }}" {{ old('seccion_agenda_id', $agendum->seccion_agenda_id) == $seccion->id ? 'selected' : '' }}>
                                    {{ $seccion->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               name="fecha_evento" 
                               value="{{ old('fecha_evento', $agendum->fecha_evento ? $agendum->fecha_evento->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <input type="time" 
                               name="hora_evento" 
                               value="{{ old('hora_evento', $agendum->hora_evento ? \Carbon\Carbon::parse($agendum->hora_evento)->format('H:i') : '') }}"
                    ...
                    <div>
                        ...
                        <select name="tipo_fijacion" 
                            ...
                            @foreach($tiposFijacion as $key => $tipo)
                                <option value="{{ $key }}" {{ old('tipo_fijacion', $agendum->tipo_fijacion) == $key ? 'selected' : '' }}>
                                    {{ $tipo }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="tipo_ventana" 
                            ...
                            @foreach($tiposVentana as $key => $ventana)
                                <option value="{{ $key }}" {{ old('tipo_ventana', $agendum->tipo_ventana) == $key ? 'selected' : '' }}>
                                    {{ $ventana }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="text" 
                               name="titulo" 
                               value="{{ old('titulo', $agendum->titulo) }}"
                    ...
                    <div>
                        ...
                        <textarea name="descripcion" 
                                  rows="6"
                                  class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#1a3b2e] focus:border-transparent"
                                  placeholder="Escriba los detalles del evento...">{{ old('descripcion', $agendum->descripcion) }}</textarea>
                    </div>
                        ...
                        <div class="mt-2">
                            <label class="inline-flex items-center">
                                <input type="checkbox" 
                                       ...
                                       {{ old('estado', $agendum->estado) ? 'checked' : '' }}>
